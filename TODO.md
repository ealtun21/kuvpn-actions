...
- Generate a macos .pkg file or .dmg file for macos, no need to ship openconnect, but if we can, great, openconnect can be installed via brew. (DONE: Added scripts/build_macos.sh and updated CI)
...
- Make build_appimage.sh use podman insternally, podman is installed in current envirment. (DONE: Added Podman wrapper to the script)
